# Simi Florin

**Software Engineer in progress, currently in Test Engineering**

Three years validating safety-critical embedded software in the automotive industry (Body Control Modules, CANoe/dSPACE/AUTOSAR, ASPICE-compliant workflows), now completing an M.Sc. in Software Engineering and building backend systems on my own time. I'm moving into a software development role — ideally one where the domain knowledge from testing (real-time systems, safety requirements, formal validation) is an asset rather than a blank slate to fill.

---

### What I'm building

**[Stocks Trading Bot](https://github.com/flosimi/stocks-trading-bot)** — An RSI+MACD trading bot running as a systemd service.
Python · pandas · broker-agnostic architecture (currently Alpaca paper trading, built to swap in Interactive Brokers for EU live trading with a single class change) · Discord notifications · remote-managed over Tailscale.

**[AI Crypto Trading Bot](#)** — Binance Spot trading with a local LLM in the loop.
React · Python · Binance API · Ollama (qwen2.5) for trade-context reasoning on top of a backtested RSI+MACD strategy · roadmap toward adaptive threshold recalibration and a reinforcement-learning agent.

**Automotive test tooling** — Internal tools automating parts of the AUTOSAR/ASPICE test workflow (test case generation via ALM/ETM REST APIs, overnight dSPACE AutomationDesk test runs, CANoe/dSPACE data sync). Built for a commercial employer, so the code stays private — happy to walk through the design in an interview.

---

### Tech

`Rust` `Python` `C#` `React` `Axum` `Qdrant` `pandas` `CANoe` `dSPACE` `AUTOSAR` `Linux` `systemd` `Tailscale`

---

### Background

- Software Test Engineer, automotive industry — Body Control Modules, ISTQB certified
- B.Sc. Computer Science, University of Craiova (2020–2024)
- M.Sc. Software Engineering, University of Craiova (2025–2027, final year)

Based in Craiova, Romania — open to relocating to Bucharest

[LinkedIn](https://www.linkedin.com/in/florin-simion-707507181/) 
